...
export async function GET(req: NextRequest) {
  try {
    ...
    const [rows] = await poolDB.query(
      `SELECT nama_mapel, materi.nama AS nama_materi, nama_kelas, materi_selesai, total_materi FROM mapel
            ...
            WHERE mapel.mapel_id = ? AND materi.status = 0 GROUP BY mapel.mapel_id ORDER BY materi.created_at ASC;`,
      [mapel]
    );
